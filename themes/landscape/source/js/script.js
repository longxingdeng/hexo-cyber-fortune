...
import iztro from 'iztro';
                    ...
                    if (addZiWeiDouShuOption) {
                        try {
                            ...
                            if (typeof iztro === 'undefined') {
                                throw new Error('紫微斗数库(iztro)未加载');
                            }
                            ...
                            const month = birthDateTime.getMonth() + 1; // JavaScript月份从0开始
                            const day = birthDateTime.getDate();
                            const hour = birthDateTime.getHours();
                            
                            // 使用iztro生成紫微斗数命盘
                            ziweiData = iztro.astro.bySolar(
                                `${year}-${month}-${day}`, 
                                Math.floor(hour), // 确保小时是整数
                                gender, 
                                true, // 是否使用真太阳时
                                'zh-CN' // 语言设置
                            );
                            ...
                            // 设置ziweiData为null，确保不会尝试使用失败的数据
                            ziweiData = null;
                        }
                    }
                    // 验证紫微斗数数据
    ...
    script.onload = function() {
        ...
        copyResultBtn.addEventListener('click', function() {
            ...
            textArea.value = llmOutputDiv.textContent;
